...
Step \ref{it:step_2} and \ref{it:step_3} in CGD algorithms, i.e. linearizing the cost and solving for the incremental warp respectively, depend on the specific optimization method used by the algorithm. In this paper, we distinguish between three main optimization methods\footnote{Notice that Amberg et al. proposed the use of the \emph{Steepest Descent} method \cite{Boyd2004} in \cite{Amberg2009}. However, their approach requires a special formulation of the motion model and it performs poorly using the standard independent AAM formulation \cite{Matthews2004} used in this work.}:
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
where, in order to unclutter the notation, we have defined $\Delta \boldsymbol{\ell} = (\Delta \mathbf{c}^T, \Delta \mathbf{p}^T)^T$ and the partial derivative of the residual with respect to the previous parameters is defined as: 
\begin{equation}
    \begin{aligned}
...
Directly inverting $\left( \frac{\partial \mathbf{r}}{\partial \Delta \boldsymbol{\ell}}^T \frac{\partial \mathbf{r}}{\partial \Delta \boldsymbol{\ell}} \right)$ has complexity $O((n + m)^3)$ for asymmetric composition and $O((2n + m)^3)$ for bidirectional composition\footnote{Notice that $m$ and $n$ denote the number of shape and appearance parameters respectively.}. However, %as noted in \cite{Papandreou2008}\footnote{Notice that the authors of \cite{Papandreou2008} applied the Schur complement using inverse composition while we apply it here using the more general \emph{asymmetric} (which includes \emph{forward}, \emph{inverse} and \emph{symmetric}) and \emph{bidirectional} compositions.}
one can take advantage of the problem structure and derive an algorithm with smaller complexity by using the \emph{Schur complement}\footnote{
Notice that that applying the Schur complement to the following system of equations:
\begin{equation*}
    \begin{aligned}
...
\begin{equation}
    \begin{aligned}
        O(\underbrace{n^3 + n^2F}_{(\mathbf{J}_{\mathbf{t}}^T\bar{\mathbf{A}}\mathbf{J}_{\mathbf{t}})^{-1}} + \underbrace{2nmF}_{\mathbf{J}_{\mathbf{t}}^T\bar{\mathbf{A}}})
...
\begin{equation}
    \begin{aligned}
        O(\underbrace{2(n)^3 + 2(n)^2F}_{
        \left( \mathbf{J}_{\mathbf{a}}^T\mathbf{P}\mathbf{J}_{\mathbf{a}} \right)^{-1} \textrm{ and } \left( \mathbf{J}_{\mathbf{i}}^T\bar{\mathbf{A}}\mathbf{J}_{\mathbf{i}} \right)^{-1}
        } 
        + 
        \underbrace{4nmF}_{
        \mathbf{J}_{\mathbf{a}}^T\mathbf{P} \textrm{ and }
        \mathbf{J}_{\mathbf{i}}^T\bar{\mathbf{A}}
        })
...
\begin{itemize}
	...
	\begin{equation}
	    ...
	    \end{aligned}
	\end{equation}
	with complexity\footnote{\label{foot:ic}Notice that, in practice, the solutions for the project-out cost function can be computed slightly faster because they do not need to explicitly solve for $\Delta\mathbf{c}$. This is specially important in the \emph{inverse} compositional case because the expressions of the form $(\mathbf{J}^T\mathbf{U}\mathbf{J})^{-1}\mathbf{J}^T\mathbf{U}$ can be completely precomputed and the complexity per iteration is reduced to $O(nF)$.} given by Equation \ref{eq:complexity_schur_asymmetric}.

	\item For \emph{bidirectional} composition:
	\begin{equation}
	    \begin{aligned}
	        \Delta \mathbf{q} & = \left( \mathbf{J}_{\mathbf{a}}^T\mathbf{P}\mathbf{J}_{\mathbf{a}} \right)^{-1} \mathbf{J}_{\mathbf{a}}^T\mathbf{P}\mathbf{r}
	        \\
...
\begin{equation}
    \begin{aligned}
        O(\underbrace{n^3 + n^2F}_{(\mathbf{J}_{\mathbf{t}}^T\mathbf{J}_{\mathbf{t}})^{-1}})
...
\item first update $\Delta\mathbf{c}$ given the previous $\Delta\mathbf{p}^*$ and $\Delta\mathbf{q}^*$ and then update $(\Delta\mathbf{p}^T, \Delta\mathbf{q}^T)^T$ from the updated $\Delta\mathbf{c}^*$, or
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
from which we can define the alternated update rule for the first of the previous two options:
\begin{equation}
    \begin{aligned}
        \Delta\mathbf{c}& = \mathbf{A}^T \left( \mathbf{r}_b + \mathbf{J}_{\mathbf{i}} \Delta\mathbf{p}^* - \mathbf{J}_{\mathbf{a}} \Delta\mathbf{q}^* \right) 
        \\
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
The rule for the second options is:
\begin{equation}
    \begin{aligned}
        \Delta\mathbf{c} & = \mathbf{A}^T \left( \mathbf{r}_b + \mathbf{J}_{\mathbf{i}} \Delta\mathbf{p}^* - \mathbf{J}_{\mathbf{a}} \Delta\mathbf{q}^* \right) 
        \\
    ...
    \end{aligned}
\end{equation}
and its complexity is dominated by:
\begin{equation}
    \begin{aligned}
        O(\underbrace{2n^3 + 2n^2F}_{(\mathbf{J}_{\mathbf{i}}^T\mathbf{J}_{\mathbf{i}})^{-1} \textrm{ and } (\mathbf{J}_{\mathbf{a}}^T\mathbf{J}_{\mathbf{a}})^{-1}})
...
\begin{itemize}
	...
	\begin{equation}
	    \begin{aligned}
	        \Delta \mathbf{q} & = \left( \mathbf{J}_{\mathbf{a}}^T\bar{\mathbf{A}}\mathbf{J}_{\mathbf{a}} \right)^{-1} \mathbf{J}_{\mathbf{a}}^T\bar{\mathbf{A}}\mathbf{r}
	        \\
	        \Delta \mathbf{p} & = -\left( \mathbf{J}_{\mathbf{i}}^T\bar{\mathbf{A}}\mathbf{J}_{\mathbf{i}} \right)^{-1} \mathbf{J}_\mathbf{i}^T \bar{\mathbf{A}} \left( \mathbf{r} + \mathbf{J}_{\mathbf{a}} \Delta\mathbf{q}^* \right)
	    \label{eq:bidirectional_alternated_po_solution}
	    \end{aligned}
	\end{equation}
	with equivalent complexity to the one given by Equation \ref{eq:complexity_schur_bidirectional2}.
\end{itemize}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
and the matrix containing the first order partial derivatives with respect to the parameters (the data term's \emph{Jacobian} matrix) can be written as:
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
On the other hand, the matrix $\frac{\partial^2 \mathcal{D}_a}{\partial^2 \Delta \boldsymbol{\ell}}$ of the second order partial derivatives (the data term's \emph{Hessian} matrix) takes the following form: 
\begin{equation}
    \begin{aligned}
...
Note that, similar to the Gauss-Newton method, the complexity of inverting the Hessian matrix $\frac{\partial^2 \mathcal{D}}{\partial^2 \Delta \boldsymbol{\ell}}$ is $O((m+2n)^3)$ for asymmetric composition and $O((2n + m)^3)$ for bidirectional composition. As pointed out by Kossaifi et al. in \cite{Kossaifi2014}\footnote{Notice that, in \cite{Kossaifi2014}, Kossaifi et al. applied the Schur complement to the Newton method using only inverse composition while we apply it here using the more general \emph{asymmetric} (which includes \emph{forward}, \emph{inverse} and \emph{symmetric}) and \emph{bidirectional} compositions.}, we can take advantage of the structure of the Hessian in Equations \ref{eq:asymmetric_newton_hessian} and \ref{eq:bidirectional_newton_hessian} and apply the Schur complement to obtain solutions with of less complexity.
...
\begin{equation}
    \begin{aligned}
        ...
        +
        \underbrace{n^2F + 4nF}_{\frac{\partial^2 \mathcal{D}_a}{\partial^2 \Delta \mathbf{p}}}
        +
        \underbrace{nmF}_{\frac{\partial^2 \mathcal{D}_a}{\partial \Delta \mathbf{p} \partial \Delta \mathbf{c}} \frac{\partial^2 \mathcal{D}_a}{\partial \Delta \mathbf{c} \partial \Delta \mathbf{p}}}
        +
        \underbrace{n^2m}_{\frac{\partial^2 \mathcal{D}_a}{\partial \Delta \mathbf{p} \partial \Delta \mathbf{c}}}
        )
...
\begin{equation}
    \begin{aligned}
    	...
    	\\
        \Delta \mathbf{p} & = \mathbf{V}^{-1} \left( \mathbf{v} - \mathbf{W} \Delta\mathbf{q}^*\right)
    	\\
        \Delta \mathbf{c} & = \frac{\partial \mathcal{D}_b}{\partial \Delta \mathbf{c}} - \frac{\partial^2 \mathcal{D}_b}{\partial \Delta \mathbf{c} \partial \Delta \mathbf{p}} \Delta \mathbf{p}^* - \frac{\partial^2 \mathcal{D}_b}{\partial \Delta \mathbf{c} \partial \Delta \mathbf{q}} \Delta \mathbf{q}^*
